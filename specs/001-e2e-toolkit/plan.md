# Implementation Plan: E2E Testing Toolkit

**Branch**: `001-e2e-toolkit` | **Date**: 2026-02-12 | **Spec**: `specs/001-e2e-toolkit/spec.md`
**Input**: Feature specification — 将现有 as-mate/e2e 测试系统重构为独立的、通用的 Docker E2E Testing Toolkit

## Summary

将 `as-mate/e2e` 的 Docker 构建、容器管理、Mock Gateway、测试运行、Dashboard 等功能提取为一个通用的、配置驱动的 monorepo 工具包 (`e2e-toolkit`)。核心价值：开发者只需提供 `Dockerfile + e2e.yaml`，即可获得完整的构建、部署、Mock、测试、调试体验。技术上采用 pnpm workspace monorepo，拆分为 `core`（引擎层）、`dashboard`（可视化层）、`cli`（命令层）三个包，通过 YAML 声明式配置统一驱动。
...
| 6 | Single Entry CLI | ✅ PASS | 统一 `e2e-toolkit <subcommand>` |
...
│   │
│   ├── dashboard/                  # @e2e-toolkit/dashboard — 可视化面板
│   │   ├── package.json
│   │   ├── tsconfig.json
│   │   ├── tsconfig.node.json
│   │   ├── vite.config.ts
...
│   │
│   └── cli/                        # @e2e-toolkit/cli — CLI 工具
│       ├── package.json
│       ├── tsconfig.json
│       ├── src/
│       │   ├── index.ts            # CLI 入口 (#!/usr/bin/env node)
│       │   ├── commands/
│       │   │   ├── init.ts         # e2e-toolkit init
│       │   │   ├── setup.ts        # e2e-toolkit setup
│       │   │   ├── build.ts        # e2e-toolkit build
│       │   │   ├── run.ts          # e2e-toolkit run
│       │   │   ├── status.ts       # e2e-toolkit status
│       │   │   ├── clean.ts        # e2e-toolkit clean
│       │   │   ├── logs.ts         # e2e-toolkit logs
│       │   │   └── dashboard.ts    # e2e-toolkit dashboard
│       │   └── utils/
...
│               └── run.test.ts
│
└── templates/                      # e2e-toolkit init 生成的模板文件
    └── default/
        ├── e2e.yaml                # 配置模板
        └── tests/
            └── health.yaml         # 示例测试
...
## Migration Mapping Summary (as-mate/e2e → e2e-toolkit)
...
| Dependency | Version | Purpose |
|-----------|---------|---------|
| `@e2e-toolkit/core` | `workspace:*` | Core engine APIs |
