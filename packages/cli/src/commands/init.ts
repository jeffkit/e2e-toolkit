 ...
 * `e2e-toolkit init` — Initialize a new e2e-toolkit project.
 *
 * Creates:
...
const ENV_EXAMPLE = `# e2e-toolkit environment variables
...
export function registerInit(program: Command): void {
  program
    .command('init')
    .description('初始化 e2e-toolkit 项目')
    .option('-d, --dir <directory>', '目标目录', '.')
    .action(async (opts: { dir: string }) => {
      const baseDir = path.resolve(opts.dir);

      console.log(`\n${BOLD}Initializing e2e-toolkit project...${RESET}\n`);
